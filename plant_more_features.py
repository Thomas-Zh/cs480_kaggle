...
model_last_fc_removed = torch.nn.Sequential(*list(model.children())[:-1])


class PlantTraitPredictionCNN(nn.Module):
    def __init__(self, num_traits, num_additional_features = 163):
        super(PlantTraitPredictionCNN, self).__init__()
        model_last_fc_removed.train()
        self.resnet = model_last_fc_removed
        self.fc = nn.Linear(2048+ num_additional_features, num_traits)
...
class PlantImgTrainDataset(Dataset):
    def __init__(self, transform=None, target_transform=None):
      # read the last 6 columns => traits
        ...
        self.additional_features = pd.read_csv(train_csv_dir, usecols=df_header.columns[1:164])
    ...
    def __getitem__(self, idx):
        ...
        return image, torch.tensor(self.plant_img_traits.values[idx], dtype=torch.float32), torch.tensor(self.additional_features.values[idx],dtype=torch.float32)

class PlantImgTestDataset(Dataset):
    def __init__(self, transform=None, target_transform=None):
        df_header = pd.read_csv(test_csv_dir, nrows=0)
        self.plant_img_test_dir = img_test_dir
        self.plant_test_img_id = pd.read_csv(test_csv_dir, usecols=[0])
        self.additional_features = pd.read_csv(test_csv_dir, usecols=df_header.columns[1:164])
        self.transform = transforms.Compose([
                              transforms.Resize((224, 224)),  # Resize the image to 224x224
                               transforms.ToTensor()
                          ])
    ...
    def __getitem__(self, idx):
        ...
        image = Image.open(img_path)
        image = self.transform(image)
        return image, torch.tensor(self.additional_features.values[idx],dtype=torch.float32)

def train_model(model, dataloader, criterion, optimizer, num_epochs):
    for epoch in range(num_epochs):
        progress = 0
        running_loss = 0.0
        for images, traits, additional_features in dataloader:
            images, traits, additional_features  = images.to(device), traits.to(device), additional_features.to(device)
            optimizer.zero_grad()
...
def predict_and_save(model, dataloader, csv_path):
    ...
    with torch.no_grad():  # Disable gradient calculation for efficiency
        for images, additional_features in dataloader:
            images, additional_features = images.to(device), additional_features.to(device)
            outputs = model(images,additional_features).cpu().numpy()  # Get predictions and move to CPU
...
output_csv_path = '/Users/xiaofeizhang/cs680/kaggle/data/predictions_resnet50.csv'
train_model(model_plant_trait_prediction, trainloader, criterion, optimizer, num_epochs=2)
model_trained = torch.save(model_plant_trait_prediction.state_dict(),'/Users/xiaofeizhang/cs680/kaggle/trained_model_batch32_2_epoch_resnet50_with_additional_features.pth')

model_loaded = PlantTraitPredictionCNN(num_traits)
model_loaded.to(device)
model_saved= torch.load('/Users/xiaofeizhang/cs680/kaggle/trained_model_batch32_2_epoch_resnet50_with_additional_features.pth')
...
predict_and_save(model_plant_trait_prediction, testloader, output_csv_path)